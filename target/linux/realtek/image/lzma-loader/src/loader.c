...
extern int board_peek(void);
static void zynos_load_dual(void);
#endif
...
#define CONFIG_FLASH_START 0xb4000000
#endif
		zynos_load_dual();
#else
		halt();
...
static void zynos_load_dual(void) {
	...
	void (*loader) (void);
	flash_base = (unsigned char *) KSEG1ADDR(CONFIG_FLASH_START);
	unsigned long flash_ofs = 0x8b0000;
	unsigned long flash_size = 0x2ffd0;
	unsigned long la;

	flash = flash_base + flash_ofs;
	la = get_be32(flash);
	loader = (void*)la;
	dest = (unsigned char *)la;
	printf("Copying from %08x bytes from %08x to %08x...\n", flash_size, (flash_base + flash_ofs + 0x30), la);
	flash += 0x30;
	for (unsigned long i=0; i<flash_size; i++)
		dest[i] = flash[i];

	flush_cache(la, flash_size);
	printf("Done copying, executing loader now ...\n");
	loader();
}

static int zynos_dual_key_pressed(void) {
	char c;
...
#if (CONFIG_ZYNOS_DUAL_IMAGE)
	printf("\nCompiled with ZyNOS Dual Image Feature\n");
	printf("\nWaiting for key press now...\n");
	res = zynos_dual_key_pressed();
	if (res != LZMA_RESULT_OK) {
		printf("Key pressed, loading secondary image\n");
		zynos_load_dual();
	}
#endif
